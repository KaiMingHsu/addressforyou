...
LIMIT 0 , 20;
 ...
 $sql1 = "SELECT * ,  ( 3959 * acos( cos( radians($lat) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($lng) ) + sin( radians($lat) ) * sin(radians(lat)) ) ) as distance FROM addressf_project.add_vill_twd97 having distance<15  order by distance limit 1 ";
...
while ($data22 = mysqli_fetch_assoc($re22)) {
	 $datas2['Maddress'][$j] =  $data22["address"];
  	$j++;
}
